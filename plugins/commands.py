...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('𖤓 Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url=GRP_LNK)
        ],[
    ...
    if len(message.command) != 2:
        ...
        if CLONE_MODE == True:
            buttons.append([InlineKeyboardButton('𖤍 Cʀᴇᴀᴛᴇ Yᴏᴜʀ Oᴡɴ Cʟᴏɴᴇ Bᴏᴛ 𖤍', callback_data='clone')])
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            if f_caption is None:
                f_caption = f"@SUPREME_BOTz {title}"
            try:
                if STREAM_MODE == True:
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"@SUPREME_BOTz {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1['file_name'].split()))}"
    ...
    if not files_:
        ...
        try:
            ...
            f_caption = f"@SUPREME_BOTz <code>{title}</code>"
            if CUSTOM_FILE_CAPTION:
                try:
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@SUPREME_BOTz {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files['file_name'].split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("ᴠᴇʀɪғʏ", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
